...
import { CREATE_KAFKA_CONNECT_MUTATION } from '@/graphql/mutations'; // Assuming this mutation exists
import type { CreateKafkaConnectMutation, CreateKafkaConnectMutationVariables } from '@/graphql/types'; // Assuming these types exist
...
export function CreateKafkaConnectForm({ clusterId, isOpen, onOpenChange, onKafkaConnectCreated }: CreateKafkaConnectFormProps) {
  ...
  const onSubmit = async (data: KafkaConnectFormData) => {
    try {
      ...
      toast.error(error.message || 'Failed to create Kafka Connect');
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Configure and create a new Kafka Connect instance for cluster <span className="font-mono">{clusterId}</span>.
          </DialogDescription>
        </DialogHeader>
