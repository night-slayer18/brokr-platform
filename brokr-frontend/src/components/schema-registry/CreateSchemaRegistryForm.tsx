...
import { CREATE_SCHEMA_REGISTRY_MUTATION } from '@/graphql/mutations'; // Assuming this mutation exists
import type { CreateSchemaRegistryMutation, CreateSchemaRegistryMutationVariables } from '@/graphql/types'; // Assuming these types exist
...
type SchemaRegistryFormData = z.infer<typeof schemaRegistrySchema>;

interface CreateSchemaRegistryFormProps {
  clusterId: string;
  isOpen: boolean;
  onOpenChange: (isOpen: boolean) => void;
  onSchemaRegistryCreated: () => void;
}

export function CreateSchemaRegistryForm({ clusterId, isOpen, onOpenChange, onSchemaRegistryCreated }: CreateSchemaRegistryFormProps) {
  ...
  const {
    ...
    setValue,
    watch,
  } = useForm<SchemaRegistryFormData>({
    resolver: zodResolver(schemaRegistrySchema),
    defaultValues: {
      name: '',
      url: '',
  ...
  const onSubmit = async (data: SchemaRegistryFormData) => {
    try {
      ...
      toast.error(error.message || 'Failed to create schema registry');
    }
  };
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Create New Schema Registry</DialogTitle>
          <DialogDescription>
            Configure and create a new Schema Registry for cluster <span className="font-mono">{clusterId}</span>.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {errors.securityProtocol && <p className="text-sm text-destructive">{errors.securityProtocol.message}</p>}
          </div>
